...
模型内置触发词不能作为预设名添加、更新或删除，例如 `gpt1`、`gpt2`、`gp1`、`gp2` 以及设置页面里模型配置的触发词。`lml` / `lmpl` 只显示用户自定义预设，不显示这些内置触发词。
...
如果没有加 `--头像`，普通文生图/预设不会把 @ 群友当参考图；`gpt2`、`bp2` 这类图生图触发词仍保留原来的 @ 头像参考行为。若消息里只有图片没有 @，顺序就是：预设 `refer_images` 在前，消息图片在后。
...
通过默认模型或模型切换使用预设时，会沿用该模型触发词的默认参数。例如默认模型是 `GPT-Image-2` 时，文生图沿用 `gpt1` 的比例设置，带图时沿用 `gpt2` 的 `auto` 图生图比例设置；手动传入 `--ar`、`--image_size`、`--model` 会优先于这些默认值。
