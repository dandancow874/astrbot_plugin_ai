    ...
    def init_providers(self):
        ...
        for provider_data in providers_data:
            ...
            for model_info in provider_data.get("models", []):
                ...
                # 使用新的字段名
                internal_provider = ProviderConfig(
...
payload = {}
                            for k, v in provider_data.items():
                                # 兼容旧的字段名
                                if k == "api_url":
                                    payload["base_url"] = self._normalize_api_url(
                                        provider_data.get("api_type", ""), v
                                    )
                                elif k == "keys" and isinstance(v, list) and v:
                                    payload["api_key"] = v[0]  # 取第一个key
                                elif k in ProviderConfig.__annotations__:
                                    payload[k] = v
...
call_params = params
            ...
            logger.info(
                f"[BIG BANANA] Dispatch check: type={provider.api_type}, url={provider.base_url}, params_model={params_model}"
            )

if provider.api_type == "OpenAI_Chat" and (
                "grsai" in (provider.base_url or "").lower()
                or "dakka.com.cn" in (provider.base_url or "").lower()
            ):
...
# 如果不是最后一个提供商，且配置了重试逻辑（隐含在列表顺序中），则继续
            if i < len(candidate_providers) - 1:
                logger.warning(
                    ...
                    f"(api_type={provider.api_type}, base_url={provider.base_url}, err={err})"
                )
